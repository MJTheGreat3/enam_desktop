...
async function loadCorporateActions() {
  console.log("[INFO] Fetching corp actions from /api/corp_actions ...");
  try {
    const res = await fetch("/api/corp_actions");
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = await res.json();
    console.log("[INFO] Loaded corp_actions records:", data.length);
    return data;
  } catch (err) {
    console.error("[ERROR] Failed to fetch corp actions:", err);
    throw err;
  }
}
